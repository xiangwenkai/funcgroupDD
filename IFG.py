...
def main():
  for ix, smiles in enumerate([
      ...
      for i, fg in enumerate(fgs):
          ...
          for j in idx:
              colors[j] = colors_s[i]
      drawer.DrawMolecule(mol, highlightAtoms=colors.keys(), highlightAtomColors=colors)
      drawer.FinishDrawing()
      svg = drawer.GetDrawingText()

      with open(f"fg_figs/mol_{ix}.svg", "w") as file:
          file.write(svg)
...
def draw_example():
    ...
    smiles = 'NC1CCCCN(C1)c2c(Cl)cc3c(=O)c(cn(C4CC4)c3c2Cl)C(=O)O'
    mol = Chem.MolFromSmiles(smiles)
    fgs = identify_functional_groups(mol)
    idxs = []
    idxt = []
    for i, fg in enumerate(fgs):
        ...
        idxs.append(idx)
        idxt.extend(idx)
    # img = Draw.MolsToGridImage([mol for _ in range(i+1)], subImgSize=(600, 600), highlightAtomLists=idxs)
    # img.show()
    img = Draw.MolsToGridImage([mol], subImgSize=(600, 600), highlightAtomLists=[idxt])
    img.show()
